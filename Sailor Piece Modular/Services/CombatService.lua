...
function CombatService:Init()
    -- Procura as remotes de ataque do jogo
...
-- 🎯 Função que o Cérebro (AutoQuest/AutoBoss) usa para dar a ordem
            ...
            if self.Target and self.Target:FindFirstChild("Humanoid") and self.Target.Humanoid.Health > 0 and hrp and hum then
                ...
                if targetHrp then
                    ...
                    if self.UseOrbit then
                        ...
                        -- Voo Fixo (Para trás/cima)
                        local pos = targetHrp.Position - (targetHrp.CFrame.LookVector * 6) + Vector3.new(0, 6, 0)
                        hrp.CFrame = CFrame.new(pos, targetHrp.Position)
                    end
                end
            else
                -- Se não tem alvo, descongela o boneco para ele não ficar flutuando à toa
                if hum then hum.PlatformStand = false end
            end
        end
    end)

    -- LOOP 2: Ataque (Porrada e Skills)
